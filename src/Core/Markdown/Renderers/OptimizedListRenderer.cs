...
/// <summary>
/// Optimized list renderer that builds Spectre.Console objects directly instead of markup strings.
/// This eliminates VT escaping issues and avoids double-parsing overhead.
/// </summary>
internal static class OptimizedListRenderer
{
        ...
        foreach (Block subBlock in item)
        {
            switch (subBlock)
            {
                ...
                case ListBlock nestedList:
                    // For nested lists, render as simple text for now
                    var nestedText = RenderNestedListAsText(nestedList, theme, 1);
                    var nestedContent = nestedText?.ToString() ?? "";
    ...
    /// <summary>
    /// Renders nested lists as indented text objects.
    /// </summary>
    private static Text RenderNestedListAsText(ListBlock list, Theme theme, int indentLevel)
    {
        var builder = new StringBuilder();
        ...
        return new Text(builder.ToString(), Style.Plain);
    }
